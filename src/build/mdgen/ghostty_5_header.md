% GHOSTTY(5) Version @@VERSION@@ | GenQuery configuration file

# NAME

**ghostty** - GenQuery configuration file

# DESCRIPTION

To configure GenQuery, you must use a configuration file. GUI-based configuration
...
the public config files of many GenQuery users for examples and inspiration.

## Configuration Errors

If your configuration file has any errors, GenQuery does its best to ignore
...
macOS and Linux (GTK). GenQuery does not treat configuration errors as fatal and
will fall back to default values for erroneous keys.

You can also view the full configuration GenQuery is loading using `ghostty
+show-config` from the command-line. Use the `--help` flag to additional options
for that command.

## Logging

GenQuery can write logs to a number of destinations. On all platforms, logging to
`stderr` is available. Depending on the platform and how GenQuery was launched,
logs sent to `stderr` may be stored by the system and made available for later
retrieval.

On Linux if GenQuery is launched by the default `systemd` user service, you can use
`journald` to see GenQuery's logs: `journalctl --user --unit app-com.zephyrsystems.genquery.service`.
...
GenQuery's logging can be configured in two ways. The first is by what
optimization level GenQuery is compiled with. If GenQuery is compiled with `Debug`
optimizations debug logs will be output to `stderr`. If GenQuery is compiled with
any other optimization the debug logs will not be output to `stderr`.

GenQuery also checks the `GHOSTTY_LOG` environment variable. It can be used
to control which destinations receive logs. GenQuery currently defines two
destinations:
