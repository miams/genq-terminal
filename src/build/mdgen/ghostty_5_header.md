% GHOSTTY(5) Version @@VERSION@@ | GenQuery Terminal configuration file

# NAME

**ghostty** - GenQuery Terminal configuration file

# DESCRIPTION

To configure GenQuery Terminal, you must use a configuration file. GUI-based configuration
...
`$HOME/Library/Application Support/com.zephyrsystems.genquery/config.ghostty`.** This is the
...
the public config files of many GenQuery Terminal users for examples and inspiration.

## Configuration Errors

If your configuration file has any errors, GenQuery Terminal does its best to ignore
...
debug output of GenQuery Terminal.
...
macOS and Linux (GTK). GenQuery Terminal does not treat configuration errors as fatal and
will fall back to default values for erroneous keys.

You can also view the full configuration GenQuery Terminal is loading using `ghostty
+show-config` from the command-line. Use the `--help` flag to additional options
for that command.

## Logging

GenQuery Terminal can write logs to a number of destinations. On all platforms, logging to
`stderr` is available. Depending on the platform and how GenQuery Terminal was launched,
logs sent to `stderr` may be stored by the system and made available for later
retrieval.

On Linux if GenQuery Terminal is launched by the default `systemd` user service, you can use
`journald` to see GenQuery Terminal's logs: `journalctl --user --unit app-com.zephyrsystems.genquery.service`.
...
--predicate 'subsystem=="com.zephyrsystems.genquery"'`.

GenQuery Terminal's logging can be configured in two ways. The first is by what
optimization level GenQuery Terminal is compiled with. If GenQuery Terminal is compiled with `Debug`
optimizations debug logs will be output to `stderr`. If GenQuery Terminal is compiled with
any other optimization the debug logs will not be output to `stderr`.

GenQuery Terminal also checks the `GHOSTTY_LOG` environment variable. It can be used
to control which destinations receive logs. GenQuery Terminal currently defines two
destinations:
